...
private:
	...
	FullGP fgp[FIELD_NUM], fixGP[FIELD_NUM];
	...
	VecDoub fgpTrain[FIELD_NUM][TRAIN_NUM];
	...
	************/
	...
	VecDoub hyp[FIELD_NUM], fixhyp[FIELD_NUM];
...
void GPBF::init()
{
  printf("start init\n");
	...
	for (int i = 0; i < field_num; i++)
	{
		...
		sprintf(str, "../dom%d/hyper_%d_new", case_id, fid_list[i]);
		fp_hyp = fopen(str, "r");
		if (fp_hyp == NULL)
		  printf("error opening %s.\n", str);
		fscanf(fp_hyp, "%d", &hyper_num[i]);
		fixhyp[i].resize(hyper_num[i]);
		for (int j = 0; j < hyper_num[i]; j++)
			fscanf(fp_hyp, "%lf", &fixhyp[i][j]);
		fclose(fp_hyp);
		printf("finish hyper\n");
		...
		sprintf(str, "../dom%d/input_%d", case_id, fid_list[i]);
		fp_in = fopen(str, "r");
		sprintf(str, "../dom%d/output_%d_new", case_id, fid_list[i]);
		fp_out = fopen(str, "r");
		for (int j = 0; j < train_num[i]; j++)
		{
			fgpTrain[i][j].resize(3);
			fscanf(fp_in, "%lf %lf", &fgpTrain[i][j][0], &fgpTrain[i][j][1]);
			fscanf(fp_out, "%lf", &fgpTrain[i][j][2]);
		}
		printf("finish read train\n");
		fclose(fp_in);
		fclose(fp_out);
...
void GPBF::FGP(VecDoub &weight, int t)
{
	...
	for (int s = 0; s < par_num; s++)
	{
		for (int f = 0; f < field_num; f++)
		{
			for (int i = 0; i < train_num[f]; i++)
			{
				ttrain[i][0] = fgpTrain[f][i][0];
				ttrain[i][1] = fgpTrain[f][i][1];
				ttrain[i][2] = fgpTrain[f][i][2];
			}
			fgp[f].predict(ttrain, train_num[f], bel_bar, pmean[s][f], pvar[s][f], par_num);
		}
	}
	getObs(weight, t);
	if ((t - 1) % SLICE_SIZE == 0)
